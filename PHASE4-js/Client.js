...
Client.prototype.sendTransaction = function(transaction, seconds) {
	if(transaction.type == "deposit" || transaction.type == "withdraw" || transaction.type == "transfer")
		this.sendTransactionToHead(transaction.bank, transaction.accountNum, transaction.amount, seconds, transaction.type, transaction.reqId, transaction.destBank, transaction.destAct);
...
Client.prototype.sendTransactionToHead = function(bank, accountNum, amount, seconds, type, reqId, destBank, destAct) {
	...
	setTimeout(function() {
		clientServer.getHeadFromMaster(bank, function(resObj) {
			...
			if(type == "transfer") {
				sendObj.destBank = destBank;
				sendObj.destAct = destAct;
			}
			sendRequest(resObj.ip, resObj.port, sendObj, null, clientServer);
		}, clientServer);
	}, 1000 * seconds);
}
